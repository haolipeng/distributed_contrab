...
1、写此设计文档的目的就是，通俗易懂的描述清楚功能需求，任何一个熟悉go语言的人都可以参照文档轻松的编写代码。
...
这些开源项目都是java语言编写的，我们可通过仿写知名的开源项目来达到练手的目的。
...
Worker集群从etcd数据库中同步master集群下发的任务（所有Worker上都是同样的任务列表），执行任务，并将任务执行结果上传到mongodb数据库。
...
1） 定时任务信息 /cron/jobs/xx

2） 强杀任务信息/cron/killer/xx

3）健康的worker节点信息 /cron/worker

etcd负责存储master下发的任务，如定时任务、强杀任务等，worker节点在启动成功后，会将自己注册到

具体存储数据结构会在后面数据结构章节详细描述。
...
master提供对外api可对任务执行增删改查操作，关联到etcd数据库的任务信息，worker从etcd数据库中读取任务信息，采取对应的操作。

### 2、4、2 任务强杀流程

### 2、4、3 健康服务的注册与发现

worker节点在健康状态下，会将自身ip注册到etcd的指定目录下/cron/worker；master从这个目录下读取全部的健康节点。

### 2、4、4 任务执行日志流程

在任务得到调度后，不管任务执行成功或失败，都会将任务执行结果存储到mongodb数据库中。具体的任务执行结果日志结构体参照下文数据结构的设计。
...
通过etcd操作/cron/jobs/目录，前期采用标准库的http库实现，后续改用gin框架来实现。
...
**健康节点发现**：从etcd数据库中读取健康的worker节点列表，此健康节点是由worker定时注册的。
...
### 3 任务调度模块(Schedler)(未完成)

### 4 任务执行模块(Executor)(未完成)
...
一个master节点，一个worker节点，此时不采用分布式锁
...
想做成一件事，最大的敌人就是要求完美，考虑的太多，做的太少，稍微考虑的差不多了，就开干吧！

可能思考时考虑的并不周全，但是并没有关系啊，好代码是不断优化出来的，发现不好的代码再优化，而不是单纯靠想出来的。

放下心中的完美主义，一次能讲代码写的完美固然好，对我来说，不断迭代出来的代码，也挺值得自己骄傲的。

写代码时，要把数据结构的字段想好，虽然后面也可以再加。



有了大致思路后，就开干，起码先把框架搭建出来，然后一点点添砖加瓦的增加功能点，就跟小孩去搭建积木一样，他们会不断寻找各种形状的积木去拼接，最终可能就拼接出来一个还算不错的玩具，比如小狗，手枪，蛋糕等等。



对我最大的感触是，自己就安排自己的思路来写。

越来越觉得，程序员最牛逼的就是把一件复杂的事情，拆分的越来越简单然后逐个击破他们。
